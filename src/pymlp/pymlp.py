...
class NeuralNetwork():
    ...
    def __init__(self, input_layer: InputLayer, hidden_layers: list[DenseLayer], output_layer: DenseOutputLayer, optimizer: Optimizer, error_metric: ErrorMetric) -> None:
        ...
        for i, layer in enumerate(self.hidden_layers):
            if i == 0:
                layer.initialize_weights(self.input_layer.nodes)
            else:
                layer.initialize_weights(self.hidden_layers[i - 1].nodes)
        self.output_layer.initialize_weights(self.hidden_layers[-1].nodes)
    ...
    def train(self, inputs: NDArray, targets: NDArray, epochs: int) -> None:
        ...
        for _ in range(1, epochs + 1):
            ...
            for i in range(inputs.shape[0]):
                ...
                cost += self.output_layer.cost_func.compute(outputs, targets[i]) + self.output_layer.regularizer.compute(self.output_layer.weights)
                # Back propagation
            ...
            # Update parameters
            for layer in self.hidden_layers:
                layer.weights_gradient /= inputs.shape[0]
                layer.biases_gradient /=inputs.shape[0]
            self.output_layer.weights_gradient /= inputs.shape[0]
            self.output_layer.biases_gradient /= inputs.shape[0]
